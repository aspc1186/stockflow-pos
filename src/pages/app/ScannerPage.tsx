...
import { CalendarClock, CheckCircle2, ClipboardCheck, PackageCheck, Pause, Trash2, Undo2 } from 'lucide-react'
...
const estadosActivos = ['en_proceso', 'abierto', 'pausado']
...
export default function ScannerPage({ modo }:{modo:'qr'|'barras'}) {
  ...
  const guardarLote = useMutation({
    mutationFn: () => api.post('/inventario', { items:lote, tipo, notas }),
    onSuccess: () => { qc.invalidateQueries({queryKey:['inventario']}); qc.invalidateQueries({queryKey:['dashboard-stats']}); setLote([]); setHistorialLote([]); setNotas(''); toast.success('Movimiento registrado y trazable') },
    onError: (e:any) => toast.error(e?.response?.data?.msg || 'No se pudo guardar el movimiento'),
  })
  const crearConteo = useMutation({
    mutationFn: () => api.post('/conteos', { accion:'crear', tipo:tipoConteo, programado_para:fecha || undefined, iniciar:!fecha }),
    onSuccess: (r:any) => {
      ...
      setFecha('')
      toast.success(nuevo?.estado === 'en_proceso' ? `${nuevo.nombre} iniciado` : `${nuevo?.nombre || 'Conteo'} programado`)
    },
  ...
  const activos = conteos.filter(item => estadosActivos.includes(item.estado))
