...
export default function ScannerPage({ modo }:{modo:'qr'|'barras'}) {
  ...
  if (modo === 'barras') return <div className="mx-auto max-w-4xl space-y-5"><div className="page-header"><div><h1 className="page-title">{tipo === 'entrada' ? 'Entrada de mercancia' : 'Salida de inventario'}</h1><p className="page-subtitle">Escanea todos los productos y confirma una sola vez. Esta salida es operativa; las ventas comerciales se registran desde Pedidos.</p></div><select className="input w-56" value={tipo} onChange={e => setTipo(e.target.value as any)}><option value="entrada">Entrada de mercancia</option><option value="salida">Salida de inventario</option></select></div><div className="card p-4"><Scanner modo="barras" continuo onDetectar={codigo => agregarAlLote(codigo, 'lote')}/>{ultimo && <p className="mt-3 text-sm text-emerald-300">Ultimo producto: {ultimo}</p>}</div>{lista(lote, setLote)}<div className="card space-y-3 p-4"><input className="input" value={notas} onChange={e => setNotas(e.target.value)} placeholder="Notas del movimiento (opcional)"/>{tipo==='entrada'&&<div><input ref={soporteCamaraRef} className="hidden" type="file" accept="image/*" capture="environment" onChange={event=>adjuntarSoporte(event.target.files?.[0])}/><input ref={soporteRef} className="hidden" type="file" accept="image/*,application/pdf" onChange={event=>adjuntarSoporte(event.target.files?.[0])}/><div className="flex flex-wrap gap-2"><button type="button" className="btn-secondary min-h-10" onClick={()=>soporteCamaraRef.current?.click()}><FileUp className="h-4 w-4"/>Tomar foto</button><button type="button" className="btn-secondary min-h-10" onClick={()=>soporteRef.current?.click()}><FileUp className="h-4 w-4"/>{soporteUrl?'Factura adjunta':'Adjuntar documento'}</button></div><p className="mt-1 text-xs text-surface-200/50">Foto, imagen o PDF, maximo 1.5 MB.</p></div>}<div className="flex flex-wrap justify-between gap-3"><button className="btn-secondary" disabled={!historialLote.length} onClick={deshacerUltimaLectura}><Undo2 className="h-4 w-4"/>Deshacer ultima lectura</button><button className="btn-primary" disabled={!lote.length || guardarLote.isPending} onClick={() => { if (window.confirm(`Confirmar ${tipo}: ${resumen.referencias} referencias y ${resumen.unidades} unidades`)) guardarLote.mutate() }}><PackageCheck className="h-4 w-4"/>{guardarLote.isPending ? 'Guardando...' : `Guardar ${tipo}`}</button></div></div></div>
