...
export default function PedidosPage() {
  ...
  const {data:pedidos=[],isLoading}=useQuery({
    ...
    queryFn:async()=>{const p=estadoQ?`?estado=${estadoQ}`:'';const {data}=await api.get<{data:Pedido[]}>(`/pedidos${p}`);return data.data},
  ...
  const filtros=[{value:'activos',label:'Activos'},{value:'cobrado',label:'Cobrados'},{value:'cancelado',label:'Cancelados'},{value:'todos',label:'Todos'}]
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex gap-2">
        {filtros.map(f=><button key={f.value} onClick={()=>setFiltro(f.value)} className={cn('btn btn-sm',filtro===f.value?'btn-primary':'btn-secondary')}>{f.label}</button>)}
      </div>
        ...
        <tbody>
          {pedidos.map(p=>{const r=p as any;return(
            <tr key={p.id} onClick={()=>navigate(`/app/pedidos/${p.id}`)} className="cursor-pointer">
              ...
              <td className="text-surface-200/70">{r.usuario_nombre??'—'}</td>
