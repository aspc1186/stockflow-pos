...
function Grafico({ data, etiqueta, clave, tipo, width, height }: { data: Record<string, any>[]; etiqueta: string; clave: string; tipo: TipoGrafico; width?: number; height?: number }) {
  ...
  if (tipo === 'dona') return <PieChart width={width} height={height}><Pie data={data} dataKey="total" nameKey={clave} innerRadius={45} outerRadius={75} paddingAngle={3}>{data.map((_, i) => <Cell key={i} fill={colores[i % colores.length]} />)}</Pie>{tooltip}</PieChart>
  if (tipo === 'radar') return <RadarChart width={width} height={height} data={data}><PolarGrid stroke="#334155" /><PolarAngleAxis dataKey={clave} tick={{ fontSize:10, fill:'#9ca3af' }} /><PolarRadiusAxis tick={false} axisLine={false} /><Radar dataKey="total" stroke="#38bdf8" fill="#38bdf8" fillOpacity={0.35} />{tooltip}</RadarChart>
  if (tipo === 'barras') return <BarChart width={width} height={height} data={data}><XAxis dataKey={clave} tick={{ fontSize:10, fill:'#9ca3af' }} interval={0} /><YAxis tick={{ fontSize:10, fill:'#9ca3af' }} tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} /><Bar dataKey="total" fill="#6366f1" radius={[5,5,0,0]} />{tooltip}</BarChart>
  if (tipo === 'linea') return <LineChart width={width} height={height} data={data}><XAxis dataKey={clave} tick={{ fontSize:10, fill:'#9ca3af' }} interval={0} /><YAxis tick={{ fontSize:10, fill:'#9ca3af' }} tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} /><Line type="monotone" dataKey="total" stroke="#38bdf8" strokeWidth={3} dot={false} />{tooltip}</LineChart>
  return <AreaChart width={width} height={height} data={data}><defs><linearGradient id={`grad-${clave}`} x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#6366f1" stopOpacity={0.35}/><stop offset="95%" stopColor="#6366f1" stopOpacity={0}/></linearGradient></defs><XAxis dataKey={clave} tick={{ fontSize:10, fill:'#9ca3af' }} interval={0} /><YAxis tick={{ fontSize:10, fill:'#9ca3af' }} tickFormatter={v => `$${(v / 1000).toFixed(0)}k`} /><Area type="monotone" dataKey="total" stroke="#6366f1" fill={`url(#grad-${clave})`} strokeWidth={2} />{tooltip}</AreaChart>
}
