...
export default function RestauranteOperacionPage({ modo }: { modo: Modo }) {
  ...
  return <div className="space-y-5">
    ...
    <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="table-base">
      <thead>{modo === 'ingredientes' ? <tr><th className="w-10"><input aria-label="Seleccionar todos los ingredientes" type="checkbox" checked={ingredientes.length>0&&seleccionados.size===ingredientes.length} onChange={event=>setSeleccionados(event.target.checked?new Set(ingredientes.map((ingrediente:any)=>ingrediente.id)):new Set())}/></th><th>Codigo</th><th>Ingrediente</th><th>Unidad compra</th><th>Unidad consumo</th><th>Factor</th><th>Precio compra ref.</th><th>Costo consumo</th><th>Stock actual</th><th>Minimo / maximo</th><th>Reorden</th><th>Merma</th><th>Rendimiento</th><th>Proveedor</th><th>Estado</th></tr> : modo === 'compras' ? <tr><th>Fecha</th><th>Proveedor</th><th>Factura</th><th>Items</th><th>Total</th><th>Soporte</th></tr> : <tr><th>Fecha</th><th>Ingrediente</th><th>Tipo</th><th>Salida</th><th>Motivo</th></tr>}</thead>
