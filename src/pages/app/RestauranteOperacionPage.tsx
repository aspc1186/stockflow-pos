...
const formatCostoUnitario = (valor: unknown) => new Intl.NumberFormat('es-CO', {
  style: 'currency',
  currency: 'COP',
  minimumFractionDigits: Number(valor || 0) % 1 ? 2 : 0,
...
export default function RestauranteOperacionPage({ modo }: { modo: Modo }) {
  ...
  const [editandoMermaId, setEditandoMermaId] = useState<string | null>(null)
  ...
  const { data: parametrosCosteo } = useQuery({
    queryKey: ['parametros-costeo-restaurante'], enabled: modo === 'costeo',
    queryFn: async () => { const { data } = await api.get<any>('/recetas?parametros=true'); return data.data || data },
  })
  const aplicarBaseCosteo = useMutation({
    mutationFn: () => api.post('/recetas?parametros=true&aplicar=true'),
    onSuccess: async (respuesta: any) => {
      await Promise.all([
        queryClient.invalidateQueries({ queryKey: ['recetas-costeo'] }),
        queryClient.invalidateQueries({ queryKey: ['productos'] }),
      ])
      const data = respuesta?.data?.data
      toast.success(`Base aplicada a ${data?.recetas_actualizadas || 0} receta(s): mano de obra ${formatCurrency(data?.mano_obra_por_porcion || 0)} e indirectos ${formatCurrency(data?.indirectos_por_porcion || 0)} por porcion.`)
    },
    onError: (error: any) => toast.error(error?.response?.data?.msg || 'No se pudo aplicar la base de costos'),
  })
  ...
  const guardarReceta = useMutation({
    mutationFn: () => api.put(`/recetas?producto_id=${recetaModal.id}`, {
      ...
      mano_obra: numero(recetaDatos.mano_obra),
  ...
  const guardar = useMutation({
    mutationFn: () => {
      if (modo === 'ingredientes') return editandoIngredienteId ? api.patch(`/ingredientes/${editandoIngredienteId}`, ingrediente) : api.post('/ingredientes', ingrediente)
      ...
      return editandoMermaId ? api.patch(`/mermas-ingredientes/${editandoMermaId}`, merma) : api.post('/mermas-ingredientes', merma)
    },
    onSuccess: async () => {
      await actualizarListas()
      setModal(false)
      setEditandoIngredienteId(null)
      setEditandoMermaId(null)
  ...
  const eliminarSeleccionados = useMutation({
    mutationFn: () => {
      if (modo === 'ingredientes') return api.delete('/ingredientes', { data: { ids: [...seleccionados] } })
      return api.delete('/recetas', { data: { producto_ids: [...seleccionados] } })
    },
    onSuccess: async (respuesta: any) => {
      const cantidad = respuesta?.data?.data?.eliminados ?? respuesta?.data?.data?.eliminadas ?? seleccionados.size
      await Promise.all([queryClient.invalidateQueries({ queryKey: ['ingredientes'] }), queryClient.invalidateQueries({ queryKey: ['productos'] })])
      setSeleccionados(new Set())
      toast.success(`${cantidad} registro${cantidad === 1 ? '' : 's'} eliminado${cantidad === 1 ? '' : 's'}`)
    },
    onError: (error: any) => toast.error(error?.response?.data?.msg || 'No se pudieron eliminar los registros'),
  })
  ...
  if (modo === 'costeo') {
    ...
    const porcionesMes = Math.max(1, Number(parametrosCosteo?.porciones_mes || 3000))
    const manoObraBase = Number(parametrosCosteo?.mano_obra_mensual || 0) / porcionesMes
    const indirectosBase = Number(parametrosCosteo?.indirectos_mensuales || 0) / porcionesMes
    return <div className="space-y-5">
      <div className="page-header"><div><h1 className="page-title">Costeo y precio sugerido</h1><p className="page-subtitle">Costo real por porcion, precio neto y precio publico con impuesto cuando aplique.</p></div><button className="btn-primary btn-sm" onClick={() => { if (window.confirm('Aplicar la base mensual de mano de obra e indirectos solo a los campos que aun estan en cero?')) aplicarBaseCosteo.mutate() }} disabled={aplicarBaseCosteo.isPending}><Calculator className="w-4 h-4" />{aplicarBaseCosteo.isPending ? 'Aplicando...' : 'Aplicar base de costos'}</button></div>
      <div className="grid gap-3 md:grid-cols-3"><div className="card p-4"><p className="text-xs uppercase text-surface-200/45">Formula de costo</p><p className="mt-1 text-sm text-surface-50">Ingredientes + mano de obra + indirectos + empaque + otros, dividido entre porciones.</p></div><div className="card p-4"><p className="text-xs uppercase text-surface-200/45">Base del Excel por porcion</p><p className="mt-1 text-sm text-surface-50">Mano de obra {formatCurrency(manoObraBase)} · Indirectos {formatCurrency(indirectosBase)}</p></div><div className="card p-4"><p className="text-xs uppercase text-surface-200/45">Precio sugerido neto</p><p className="mt-1 text-sm text-surface-50">Costo x (1 + recargo sobre costo). El IVA o INC se agrega aparte si el precio lo incluye.</p></div></div>
      <p className="text-sm text-surface-200/60">El empaque queda en $0 hasta que lo configures en cada receta: no todos los platos lo requieren. La base usa {porcionesMes.toLocaleString('es-CO')} porciones mensuales.</p>
      ...
      <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="table-base"><thead><tr><th>Codigo</th><th>Plato</th><th>Ingredientes<br/>por porcion</th><th>Mano de obra<br/>por porcion</th><th>Indirectos<br/>por porcion</th><th>Empaque<br/>por porcion</th><th>Otros<br/>por porcion</th><th>Costo real<br/>por porcion</th><th>Recargo</th><th>Precio neto</th><th>Impuesto</th><th>Precio publico</th><th></th></tr></thead><tbody>{recetasFiltradas.map((receta: any) => {
        const porciones = Math.max(1, Number(receta.porciones || 1))
        const porcion = (valor: unknown) => Number(valor || 0) / porciones
        ...
        const netoRedondeado = Math.ceil(sugerido / redondeo) * redondeo
        const impuestoPct = Math.max(0, Number(receta.impuesto_pct || 0))
        const publico = receta.impuesto_incluido ? Math.ceil((netoRedondeado * (1 + impuestoPct / 100)) / redondeo) * redondeo : netoRedondeado
        return <tr key={receta.id}><td className="font-mono text-xs text-surface-200/60">{receta.producto_codigo || '-'}</td><td className="font-medium">{receta.producto_nombre}<span className="block text-xs font-normal text-surface-200/45">{porciones} porcion{porciones === 1 ? '' : 'es'} por receta</span></td><td>{formatCurrency(porcion(receta.costo_ingredientes))}</td><td>{formatCurrency(porcion(receta.mano_obra))}</td><td>{formatCurrency(porcion(receta.costos_indirectos))}</td><td>{formatCurrency(porcion(receta.empaque))}</td><td>{formatCurrency(porcion(receta.otros_costos))}</td><td className="font-semibold">{formatCurrency(costo)}</td><td>{porcentaje}%<span className="block text-xs text-surface-200/45">sobre costo</span></td><td>{formatCurrency(netoRedondeado)}</td><td>{impuestoPct ? `${String(receta.impuesto_tipo || 'impuesto').toUpperCase()} ${impuestoPct}%` : '-'}</td><td className="font-semibold text-emerald-300">{formatCurrency(publico)}{receta.impuesto_incluido && impuestoPct ? <span className="block text-xs font-normal text-surface-200/45">impuesto incluido</span> : null}</td><td><button className="btn-secondary btn-sm" onClick={() => abrirReceta({ id: receta.producto_id, nombre: receta.producto_nombre })}><Pencil className="w-4 h-4" />Configurar</button></td></tr>
      })}{recetasFiltradas.length===0&&<tr><td colSpan={13} className="py-12 text-center text-surface-200/40">Crea o importa recetas para calcular el costo y precio sugerido.</td></tr>}</tbody></table></div></div>
    </div>
  }
  ...
  const registrosFiltrados = registros.filter((registro: any) => normalizar([registro.nombre, registro.codigo, registro.categoria, registro.ingrediente_nombre, registro.proveedor, registro.numero_factura, registro.motivo].join(' ')).includes(normalizar(busqueda)))
  const abrirEdicionIngrediente = (registro: any) => {
    setEditandoIngredienteId(registro.id)
    setIngrediente({ nombre:registro.nombre || '', codigo:registro.codigo || '', categoria:registro.categoria || '', unidad_compra:registro.unidad_compra || 'unidad', unidad_consumo:registro.unidad_consumo || 'unidad', factor_conversion:String(registro.factor_conversion || 1), stock_inicial:String(registro.stock_actual || 0), costo_unitario:String(registro.costo_unitario || 0), stock_minimo:String(registro.stock_minimo || 0), stock_maximo:registro.stock_maximo == null ? '' : String(registro.stock_maximo), punto_reorden:registro.punto_reorden == null ? '' : String(registro.punto_reorden), proveedor_principal:registro.proveedor_principal || '' })
    setModal(true)
  }
  ...
  return <div className="space-y-5">
    <div className="page-header">
      ...
      <div className="flex flex-wrap gap-2">
        {modo === 'ingredientes' && <>
          ...
          {seleccionados.size>0&&<button className="btn-danger btn-sm" disabled={eliminarSeleccionados.isPending} onClick={()=>{if(window.confirm(`Eliminar ${seleccionados.size} ingrediente(s) seleccionados? Se conservara el historial.`))eliminarSeleccionados.mutate()}}><Trash2 className="w-4 h-4" />{eliminarSeleccionados.isPending?'Eliminando...':`Eliminar (${seleccionados.size})`}</button>}
        </>}
          <button className="btn-primary btn-sm" onClick={() => { setEditandoIngredienteId(null); setEditandoMermaId(null); setModal(true) }}><Plus className="w-4 h-4" />{modo === 'ingredientes' ? 'Nuevo ingrediente' : modo === 'compras' ? 'Registrar compra' : 'Registrar movimiento'}</button>
      </div>
    </div>
    ...
    {modo === 'ingredientes' && <div className="rounded-lg border border-brand-400/20 bg-brand-500/5 px-4 py-3 text-sm text-surface-200/75"><strong className="text-surface-50">Trazabilidad del costo:</strong> el <strong>precio de compra ref.</strong> equivale al costo unitario de consumo multiplicado por el factor de conversión. Por ejemplo, $5.500 por kilogramo equivale a $5,50 por gramo. El stock se mueve con entradas, ventas y ajustes; cada compra recalcula el costo de consumo por promedio ponderado y actualiza las recetas.</div>}
    <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="table-base">
      <thead>{modo === 'ingredientes' ? <tr><th className="w-10"><input aria-label="Seleccionar todos los ingredientes" type="checkbox" checked={ingredientes.length>0&&seleccionados.size===ingredientes.length} onChange={event=>setSeleccionados(event.target.checked?new Set(ingredientes.map((ingrediente:any)=>ingrediente.id)):new Set())}/></th><th>Codigo</th><th>Ingrediente</th><th>Unidad compra</th><th>Unidad consumo</th><th>Factor</th><th>Precio compra ref.</th><th>Costo consumo</th><th>Stock actual</th><th>Minimo / maximo</th><th>Reorden</th><th>Merma</th><th>Rendimiento</th><th>Proveedor</th><th>Estado</th><th></th></tr> : modo === 'compras' ? <tr><th>Fecha</th><th>Proveedor</th><th>Factura</th><th>Items</th><th>Total</th><th>Soporte</th></tr> : <tr><th>Fecha</th><th>Ingrediente</th><th>Tipo</th><th>Salida</th><th>Motivo</th><th></th></tr>}</thead>
      <tbody>{registrosFiltrados.map((registro: any) => {
        if (modo === 'ingredientes') {
          const factorConversion = Math.max(1, Number(registro.factor_conversion || 1))
          const costoConsumo = Number(registro.costo_unitario || 0)
          const precioCompraReferencia = costoConsumo * factorConversion
          const stockMinimo = registro.stock_minimo === null || registro.stock_minimo === undefined ? '-' : Number(registro.stock_minimo).toFixed(3)
          const stockMaximo = registro.stock_maximo === null || registro.stock_maximo === undefined ? '-' : Number(registro.stock_maximo).toFixed(3)
          const merma = registro.merma_pct === null || registro.merma_pct === undefined ? '-' : `${Number(registro.merma_pct)}%`
          const rendimiento = registro.rendimiento === null || registro.rendimiento === undefined ? '-' : `${Number(registro.rendimiento) * 100}%`
          return <tr key={registro.id}><td><input aria-label={`Seleccionar ${registro.nombre}`} type="checkbox" checked={seleccionados.has(registro.id)} onChange={event=>setSeleccionados(actual=>{const siguiente=new Set(actual);if(event.target.checked)siguiente.add(registro.id);else siguiente.delete(registro.id);return siguiente})}/></td><td className="font-mono text-xs text-surface-200/60">{registro.codigo || '-'}</td><td><p className="font-medium">{registro.nombre}</p><p className="text-xs text-surface-200/40">{registro.categoria || 'Sin categoria'}</p></td><td>{registro.unidad_compra || '-'}</td><td>{registro.unidad_consumo || '-'}</td><td>{factorConversion.toLocaleString('es-CO')}</td><td className="font-medium">{formatCostoUnitario(precioCompraReferencia)}</td><td className="font-medium text-emerald-300">{formatCostoUnitario(costoConsumo)}</td><td className={Number(registro.stock_actual) <= Number(registro.stock_minimo) ? 'text-red-400' : 'text-surface-50'}>{Number(registro.stock_actual || 0).toFixed(3)}</td><td>{stockMinimo} / {stockMaximo}</td><td>{registro.punto_reorden === null || registro.punto_reorden === undefined ? '-' : Number(registro.punto_reorden).toFixed(3)}</td><td>{merma}</td><td>{rendimiento}</td><td>{registro.proveedor_principal || '-'}</td><td><span className={registro.activo ? 'badge-green' : 'badge-gray'}>{registro.activo ? 'Activo' : 'Inactivo'}</span></td><td><button type="button" className="btn-ghost btn-sm p-2" title="Editar ingrediente" onClick={() => abrirEdicionIngrediente(registro)}><Pencil className="h-4 w-4"/></button></td></tr>
        }
        ...
        return <tr key={registro.id}><td>{formatDate(registro.created_at, 'dd/MM/yy HH:mm')}</td><td>{registro.ingrediente_nombre}</td><td className="capitalize">{registro.tipo.replace('_', ' ')}</td><td className={Number(registro.entrada || 0) > 0 ? 'text-emerald-400' : 'text-red-400'}>{Number(registro.entrada || 0) > 0 ? '+' : '-'}{Number(registro.entrada || registro.salida || 0)}</td><td>{registro.motivo || '-'}</td><td><button type="button" className="btn-ghost btn-sm p-2" title="Editar motivo" onClick={() => { setEditandoMermaId(registro.id); setMerma({ ingrediente_id:registro.ingrediente_id, cantidad:String(Number(registro.entrada || registro.salida || 0)), tipo:registro.tipo, motivo:registro.motivo || '', observaciones:registro.observaciones || '' }); setModal(true) }}><Pencil className="h-4 w-4"/></button></td></tr>
      })}</tbody>
    </table></div></div>
    <Modal open={modal} onClose={() => setModal(false)} title={modo === 'ingredientes' ? (editandoIngredienteId ? 'Editar ingrediente' : 'Nuevo ingrediente') : modo === 'compras' ? 'Registrar compra' : editandoMermaId ? 'Editar detalle del movimiento' : 'Registrar merma o ajuste'} footer={<div className="flex gap-3"><button className="btn-secondary flex-1" onClick={() => setModal(false)}>Cancelar</button><button className="btn-primary flex-1" onClick={() => guardar.mutate()} disabled={guardar.isPending}>{guardar.isPending ? 'Guardando...' : 'Guardar'}</button></div>}>
      {modo === 'ingredientes' ? <div className="grid grid-cols-2 gap-3">
        ...
        <div><label className="label">Stock inicial</label><input type="number" min="0" className="input disabled:opacity-50" disabled={!!editandoIngredienteId} value={ingrediente.stock_inicial} onChange={(event) => setIngrediente((actual) => ({ ...actual, stock_inicial: event.target.value }))} />{editandoIngredienteId&&<p className="mt-1 text-xs text-surface-200/45">El stock se corrige desde Mermas y ajustes.</p>}</div>
        <div><label className="label">Costo por unidad de consumo</label><input type="number" min="0" className="input disabled:opacity-50" disabled={!!editandoIngredienteId} value={ingrediente.costo_unitario} onChange={(event) => setIngrediente((actual) => ({ ...actual, costo_unitario: event.target.value }))} />{editandoIngredienteId&&<p className="mt-1 text-xs text-surface-200/45">El costo se actualiza al registrar una compra.</p>}</div>
