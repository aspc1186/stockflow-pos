  ...
  return <div className="space-y-5">
    ...
    <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="table-base">
      ...
      <tbody>{registrosFiltrados.map((registro: any) => {
        if (modo === 'ingredientes') return <tr key={registro.id}><td><input aria-label={`Seleccionar ${registro.nombre}`} type="checkbox" checked={seleccionados.has(registro.id)} onChange={event=>setSeleccionados(actual=>{const siguiente=new Set(actual);if(event.target.checked)siguiente.add(registro.id);else siguiente.delete(registro.id);return siguiente})}/></td><td><p className="font-medium">{registro.nombre}</p><p className="text-xs text-surface-200/40">{registro.codigo || 'Sin codigo'}</p></td><td>{registro.unidad_consumo}</td><td className={Number(registro.stock_actual) <= Number(registro.stock_minimo) ? 'text-red-400' : 'text-surface-50'}>{Number(registro.stock_actual || 0).toFixed(3)}</td><td>{formatCurrency(registro.costo_unitario || 0)}</td><td>{registro.punto_reorden || registro.stock_minimo || 0}</td><td><span className={registro.activo ? 'badge-green' : 'badge-gray'}>{registro.activo ? 'Activo' : 'Inactivo'}</span></td></tr>
