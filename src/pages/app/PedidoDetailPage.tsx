...
import type { Pedido, Producto, PedidoItem } from '@/types'
...
export default function PedidoDetailPage() {
  ...
  const {data:pedido,isLoading}=useQuery({
    queryKey:['pedido',id],
    queryFn:async()=>{const {data}=await api.get<{data:Pedido}>(`/pedidos/${id}`);return data.data},
  })
  const {data:productos=[]}=useQuery({
    queryKey:['productos'],
    queryFn:async()=>{const {data}=await api.get<{data:Producto[]}>('/productos?disponible=true');return data.data},
    enabled:modalP,
  })
  ...
  const cobrar=async()=>{
    try {
      ...
      if(p>0) await api.post('/caja',{accion:'movimiento',tipo:'propina',metodo_pago:metodo,monto:p,descripcion:`Propina #${pedido?.numero}`,pedido_id:id})
  ...
  const p=pedido as any; const activo=!['cobrado','cancelado'].includes(pedido.estado)

  return (
    <div className="space-y-5 max-w-2xl">
      <div className="flex items-center gap-3">
        <button onClick={()=>navigate(-1)} className="btn-ghost btn-sm p-2"><ArrowLeft className="w-4 h-4"/></button>
        <div className="flex-1"><h1 className="page-title">Pedido #{pedido.numero}</h1><p className="page-subtitle">{p.mesa_numero?`Mesa ${p.mesa_numero} · `:''}{p.usuario_nombre} · {formatDate(pedido.apertura_at,'HH:mm')}</p></div>
        ...
        <div className="divide-y divide-white/5">
          {pedido.items?.map(item=>{const it=item as any;return(
            <div key={item.id} className="flex items-center gap-4 px-5 py-3.5">
              <div className="flex-1 min-w-0"><p className="text-sm font-medium text-surface-50">{it.nombre??item.producto_id}</p>{item.observaciones&&<p className="text-xs text-surface-200/50 mt-0.5">{item.observaciones}</p>}</div>
              ...
              <span className={`badge text-[10px] ${item.estado==='pendiente'?'badge-yellow':item.estado==='en_preparacion'?'badge-blue':item.estado==='listo'?'badge-green':'badge-gray'}`}>{item.estado}</span>
            </div>
          )})}
      ...
      <Modal open={modalC} onClose={()=>setModalC(false)} title="Cobrar pedido" size="sm"
        ...
        <div className="space-y-4">
          <div><label className="label">Método de pago</label><select className="input" value={metodo} onChange={e=>setMetodo(e.target.value)}>{['efectivo','tarjeta','transferencia','nequi','daviplata','mixto'].map(m=><option key={m} value={m} className="bg-surface-800 capitalize">{m}</option>)}</select></div>
      ...
      <ConfirmDialog open={confirmCan} onClose={()=>setConfirmCan(false)}
        ...
        title="Cancelar pedido" message="¿Cancelar este pedido? No se puede deshacer." confirmLabel="Sí, cancelar" danger/>
    </div>
  )
}
