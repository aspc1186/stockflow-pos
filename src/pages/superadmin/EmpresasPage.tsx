...
export default function EmpresasPage() {
  ...
  const { data: empresas = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data.data || data) as Empresa[]
    },
  ...
  const crear = useMutation({
    ...
    onError: (e: any) => toast.error(e?.response?.data?.msg ?? e?.response?.data?.message ?? 'Error al crear empresa'),
  })
          ...
          <tbody>
            {filtradas.length === 0 ? (
              ...
              <tr key={empresa.id} className="border-b border-white/5 hover:bg-white/2 transition-colors">
                ...
                <td className="p-4">
                  <button
                    onClick={() => toggle.mutate({ id: empresa.id, activa: !empresa.activa })}
                ...
                <td className="p-4">
                  <button onClick={() => navigate(`/superadmin/empresas/${empresa.id}`)} className="btn-ghost btn-sm">
        ...
        <div className="space-y-5">
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input className="input" placeholder="Nombre del establecimiento" value={form.nombre} onChange={e => setForm(p => ({ ...p, nombre: e.target.value }))} />
              </div>
              <div>
              ...
              <div>
                ...
                <input className="input" placeholder="Ciudad" value={form.ciudad} onChange={e => setForm(p => ({ ...p, ciudad: e.target.value }))} />
              </div>
              <div>
                <label className="label">NIT</label>
                <input className="input" placeholder="NIT o documento" value={form.nit} onChange={e => setForm(p => ({ ...p, nit: e.target.value }))} />
              </div>
              <div>
                <label className="label">Licencia hasta</label>
                <input type="date" className="input" value={form.licencia_fin} onChange={e => setForm(p => ({ ...p, licencia_fin: e.target.value }))} />
              </div>
            </div>
          </div>
          <div className="border-t border-white/5 pt-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="label">Nombre</label>
                <input className="input" placeholder="Nombre completo" value={form.admin_nombre} onChange={e => setForm(p => ({ ...p, admin_nombre: e.target.value }))} />
              </div>
              <div>
                <label className="label">Username</label>
                <input className="input" placeholder="Usuario de acceso" value={form.admin_username} onChange={e => setForm(p => ({ ...p, admin_username: e.target.value }))} />
              </div>
              <div>
                <label className="label">Email *</label>
                <input type="email" className="input" placeholder="correo@ejemplo.com" value={form.admin_email} onChange={e => setForm(p => ({ ...p, admin_email: e.target.value }))} />
              </div>
              <div>
                <label className="label">Contraseña *</label>
                <input type="password" className="input" placeholder="Mínimo 8 caracteres" value={form.admin_password} onChange={e => setForm(p => ({ ...p, admin_password: e.target.value }))} />
              </div>
            </div>
          </div>
        </div>
