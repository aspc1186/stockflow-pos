...
export default function MesaPedidoPage() {
  ...
  const {data:mesa}=useQuery({queryKey:['mesa',mesaId],queryFn:async()=>{const {data}=await api.get<{data:Mesa}>(`/mesas/${mesaId}`);return data.data}})
  const {data:cats=[]}=useQuery({queryKey:['categorias'],queryFn:async()=>{const {data}=await api.get<{data:Categoria[]}>('/categorias');return data.data}})
  const {data:productos=[]}=useQuery({queryKey:['productos'],queryFn:async()=>{const {data}=await api.get<{data:Producto[]}>('/productos?disponible=true');return data.data}})
  const {data:pedidoActivo}=useQuery({queryKey:['pedido-activo',mesaId],queryFn:async()=>{const {data}=await api.get<{data:Pedido[]}>(`/pedidos?mesa_id=${mesaId}&estado=abierto,en_preparacion,listo`);return data.data[0]??null}})
  ...
  const enviar=async()=>{
    ...
    try {
      ...
      if(pedidoActivo){await api.post(`/pedidos/${pedidoActivo.id}`,{items})}
      ...
      <div className="sticky top-0 z-10 bg-surface-800/90 backdrop-blur border-b border-white/5 px-4 py-3">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-surface-200/50">{pedidoActivo?`Pedido activo · ${formatCurrency(pedidoActivo.total)}`:'Sin pedido'}</p>
          </div>
          {pedidoActivo&&<button onClick={async()=>{try{await api.patch(`/pedidos/${pedidoActivo.id}`,{estado:'lista_cobrar'});await api.patch(`/mesas/${mesaId}`,{estado:'lista_cobrar'});toast.success('Cuenta solicitada')}catch{toast.error('Error')}}} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-orange-500/20 text-orange-400 text-xs font-medium"><CreditCard className="w-3.5 h-3.5"/>Cuenta</button>}
        </div>
      </div>
