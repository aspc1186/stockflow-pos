...
export default function Scanner({ modo, onDetectar, continuo=false }:ScannerProps) {
  ...
  useEffect(()=>()=>detener(),[])
  return <div className="space-y-3"><div className="mx-auto w-full max-w-[320px] overflow-hidden rounded-lg border border-white/10 bg-black/30"><video ref={videoRef} muted playsInline className="h-28 w-full object-cover sm:h-36"/><div className="border-t border-white/10 px-3 py-2 text-xs text-surface-200/65">{mensaje}</div></div><div className="flex flex-wrap gap-2"><button type="button" className="btn-primary min-h-10" onClick={iniciar}><Camera className="h-4 w-4"/>{camaraActiva?'Reiniciar camara':'Usar camara'}</button>{camaraActiva&&<button type="button" className="btn-secondary min-h-10" onClick={detener}>Detener</button>}</div><div className="flex gap-2"><div className="relative flex-1"><Keyboard className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-surface-200/45"/><input autoFocus className="input min-h-10 pl-10" value={manual} onChange={event=>setManual(event.target.value)} onKeyDown={event=>{if(event.key==='Enter') confirmar(manual)}} placeholder={modo==='qr'?'Pega o escanea el QR':'Escribe o escanea el codigo de barras'}/></div><button type="button" className="btn-secondary min-h-10" onClick={()=>confirmar(manual)}><ScanLine className="h-4 w-4"/>Validar</button></div></div>
}
