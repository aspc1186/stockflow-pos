...
const groups = [
  ...
  { label:'Estaciones', items:[
    {to:'/cocina',label:'Cocina',icon:ChefHat,roles:['admin','cocinero','supervisor']},
    {to:'/barra',label:'Barra',icon:Wine,roles:['admin','bartender','supervisor']},
  ]},
  { label:'Gestión', items:[
    {to:'/app/productos',label:'Productos',icon:Package,roles:['admin','supervisor']},
    {to:'/app/inventario',label:'Inventario',icon:Package,roles:['admin','supervisor','bodeguero']},
    {to:'/app/compras',label:'Compras',icon:ShoppingCart,roles:['admin','supervisor','bodeguero']},
    ...
    <div className="w-60 h-full bg-surface-800 border-r border-white/5 flex flex-col">
      ...
      <div className="p-3 border-t border-white/5">
        <div className="flex items-center gap-2.5 px-3 py-2 mb-1">
          <div className="w-7 h-7 rounded-full bg-brand-600/30 flex items-center justify-center"><span className="text-[11px] font-bold text-brand-300">{user?.nombre?.[0]??'U'}</span></div>
          <div className="min-w-0"><p className="text-xs font-medium text-surface-50 truncate">{user?.nombre}</p><p className="text-[10px] text-surface-200/40 capitalize">{user?.rol?.nombre}</p></div>
        </div>
