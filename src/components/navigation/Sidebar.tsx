...
import { LayoutDashboard, UtensilsCrossed, ClipboardList, Package, Users, BarChart3, Settings, CreditCard, X, ChevronDown, Phone, ContactRound, Plug, CookingPot, ShoppingCart, BadgeAlert, BookOpen, CalendarDays, PartyPopper, Calculator } from 'lucide-react'
...
export default function Sidebar({onClose}:{onClose?:()=>void}){
  ...
  return <div className="w-60 h-full bg-surface-800 border-r border-white/5 flex flex-col">
    ...
    <nav className="flex-1 overflow-y-auto p-3 space-y-4 scrollbar-none">
      ...
      {esRestaurante && <div><p className="px-3 mb-1.5 text-[10px] font-semibold text-surface-200/30 uppercase tracking-widest">Restaurante</p><div className="space-y-0.5">{[{to:'/app/ingredientes',label:'Ingredientes',icon:CookingPot},{to:'/app/recetas',label:'Recetas',icon:BookOpen},{to:'/app/costeo',label:'Costeo y precios',icon:Calculator},{to:'/app/compras-ingredientes',label:'Compras',icon:ShoppingCart},{to:'/app/mermas-ingredientes',label:'Mermas y ajustes',icon:BadgeAlert}].map(item=><NavLink key={item.to} to={item.to} onClick={onClose} className={({isActive})=>cn('sidebar-link',isActive&&'active')}><item.icon className="w-4 h-4 flex-shrink-0"/><span className="truncate">{item.label}</span></NavLink>)}</div></div>}
    </nav>
