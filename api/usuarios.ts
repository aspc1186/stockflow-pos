...
module.exports = async function handler(req: any, res: any) {
  ...
  if (!id) {
    if (req.method === 'GET') {
      const rows = await query(`SELECT u.id,u.nombre,u.email,u.username,u.telefono,u.activo,u.ultimo_acceso,u.created_at,json_build_object('id',r.id,'nombre',r.nombre) as rol FROM usuarios u JOIN roles r ON r.id=u.rol_id WHERE u.empresa_id=$1 ORDER BY u.nombre`, [eid])
    ...
    if (req.method === 'POST') {
      ...
      try {
        const [u] = await query(`INSERT INTO usuarios (id,empresa_id,rol_id,nombre,email,username,password_hash,telefono) VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING id,nombre,email,username,activo`, [uuid(), eid, rol_id, nombre, email.toLowerCase(), uname, hash, telefono || null])
