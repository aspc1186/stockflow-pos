...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    const rows = await query(`SELECT * FROM categorias WHERE empresa_id=$1 ORDER BY nombre`,[eid])
  ...
  if (req.method === 'POST') {
    ...
    const [row]=await query(`INSERT INTO categorias (${keys.join(',')}) VALUES (${placeholders}) RETURNING *`,vals)
