...
export default async function handler(req: any, res: any) {
  ...
  if (req.method==='POST') {
    ...
    if (accion==='cerrar') {
      ...
      const sf=parseFloat(caja.saldo_inicial)+parseFloat(caja.total_ventas)+parseFloat(caja.total_ingresos)-parseFloat(caja.total_egresos)-parseFloat(caja.total_compras_inventario || 0)
