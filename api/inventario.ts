...
module.exports = async function handler(req: any, res: any) {
  ...
  if (req.method === 'GET') {
    ...
    const rows = await query(`SELECT i.*,p.nombre as producto_nombre,p.codigo,p.unidad_medida FROM inventario i JOIN productos p ON p.id=i.producto_id WHERE ${where} ORDER BY p.nombre`, params)
  ...
  if (req.method === 'POST') {
    ...
    const inv = await queryOne(`SELECT stock_actual FROM inventario WHERE producto_id=$1 AND empresa_id=$2`, [producto_id, eid])
    if (!inv) return res.status(404).json({ success: false, message: 'Sin inventario' })
    const q = parseFloat(String(cantidad)) || 0
    const antes = inv.stock_actual
    let despues = antes
    ...
    await query(`INSERT INTO movimientos_inventario (id,empresa_id,producto_id,usuario_id,tipo,cantidad,stock_antes,stock_despues,notas) VALUES (gen_random_uuid(),$1,$2,$3,$4,$5,$6,$7,$8)`, [eid, producto_id, auth.id, tipo, Math.abs(q), antes, despues, notas || null])
    return res.status(201).json({ success: true, data: { producto_id, tipo, cantidad: q, stock_antes: antes, stock_despues: despues } })
  }
