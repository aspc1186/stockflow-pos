...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (req.method === 'GET') {
    ...
    const rows=await query(`SELECT i.*,p.nombre as producto_nombre,p.codigo,p.unidad_medida,c.nombre as categoria_nombre FROM inventario i JOIN productos p ON p.id=i.producto_id LEFT JOIN categorias c ON c.id=p.categoria_id WHERE ${where} ORDER BY p.nombre`,params)
